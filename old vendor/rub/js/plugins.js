...
var link = {
	init: function () {
		$("[aria-link], [data-link]").each(function (i, el) {
			$(el).attr('tabindex', 0);
		});
		$("[aria-link], [data-link]").off('click.links').on('click.links', function (e) {
			var url = $(this).attr('aria-link') || $(this).attr('data-link'),
...
var popup = {
	...
	HEIGHT: 50,
	/**
	 * @see  <optional-params>  popup.init(width, height, top, left)
	 */
	init: function (w, h, t, l) {
		...
		$('[data-modal].popup').each(function (i, el) {
			...
			if (dataset.block === false) {
				...
				if ($(el).closest('.modalOverlay').length > 0) {
					$(el).closest('.modalOverlay').removeClass(toggleclasses);
				}
			}
			if ($(el).closest('.modalOverlay').length < 1) {
				$(el).wrap('<div class="modalOverlay ' + of + '">&nbsp;</div>');
			}
		});
		...
		$('[data-popup]').on('click', function (e) {
			...
			// if( !(dataset.hasOwnProperty("offsetX")) ){
			// 	$(thisOb).attr('data-offset-x', 0);
			// 	dataset.offsetX = 0;
			// }
			// if( !(dataset.hasOwnProperty("offsetY")) ) {
			// 	$(thisOb).attr('data-offset-y', 0);
			// 	dataset.offsetY = 0;
			// }
			...
			if (dataset.position === 'inline' || $(ob).attr('data-position') === 'inline') {
				...
				// popup.WIDTH = w;
				// popup.HEIGHT = h;
				// console.log('init :>) %d X %d on X:%d and Y: %d', w, h, l, t);
			}
	...
	open: function (obPop, w, h) {
		...
		$(ob).attr('data-offset-x', dataset.offsetX);
		$(ob).attr('data-offset-y', dataset.offsetY);
			...
			for (key in vArgs[i]) {
				...
				if (vArgs[i].hasOwnProperty(key)) {
					if (key.match(/^xsource/)) {
						...
						$(obPop).find('.popContent').html('<iframe id="' + obPop + '_iframe" onload="popup.loader(\'' + obPop + '\',false)" src="' + vArgs[i][key] + '" frameborder="0"></iframe>');
					}
					...
					// --ENDS will be DEPRECATED on future versions
						...
						if (ds.datatype === 'JSON_encoded' && typeof (ds.data === 'String')) {
							if ($(ds.data).length > 0) {
								var json = decode($(ds.data).val(), 'json_object');
								// console.log(dat);
						...
						// }
							...
							// console.log(json);
							this.setData(obPop, json);
						} else if (ds.datatype === 'JSON_object') {
							var json = ds.data;
							// console.log(json);
							this.setData(obPop, json);
						}

						// $('[data-get]').each( function (ei, el) {
						// });
					}
				}
			}
		...
		if (dataset.units === 'px' || dataset.units === 'pixel') {
			...
			if (dataset.position === 'center') {
				...
				$(ob).closest('.modalOverlay').addClass('inlineMock');
				popup.TOP = t;
				popup.LEFT = l;
			}
			$(ob).attr({
		...
		if (dataset.block === true || dataset.block === undefined) {
		...
		if ($.trim(dataset.animateIn) !== '') {
			$(ob).addClass('open ' + dataset.class + ' ' + dataset.animateIn)
				.one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
					$(this).removeClass(dataset.animateIn);
					popup.refresh();
				}).parent('.modalOverlay').addClass('active').css({
					// 'display': 'block',
					// 'visibility': 'visible',
					'z-index': 1050 + dataset.level
				});
		} else {
			$(ob).addClass('open ' + dataset.class).parent('.modalOverlay').addClass('active').css({
				// 'display': 'block',
				// 'visibility': 'visible',
				'z-index': 1050 + dataset.level
			});
		...
		popup.refresh(ob);
		$(obPop).trigger("popup.open", $(obPop));
	},
	refresh: function () {
		"use strict";
		...
		$(ob).each(function (i, el) {
			...
			hdr = $(el).find('.popHeader').innerHeight();
			ftr = $(el).find('.popFooter').innerHeight();
			$(el).find('.popContent').css('height', $(el).innerHeight() - (hdr + ftr));
	...
	setData: function (obPop, json) {
		$(json).each(function (di, dx) {
			...
			if (dx.hasOwnProperty(k)) {
				// console.log(k, dx[k]);
				if (dx.hasOwnProperty('attr')) { // k === 'selector'
					if ($(obPop).find(dx[k]).length > 0) {
						if ($(dx[k]).is('input:checkbox') || $(dx[k]).is('input:radio')) {
							$(obPop).find(dx[k])[0].checked = true;
							// console.log($(obPop).find(dx[k]).is(':checked'));
						} else if ($(dx[k]).is('[data-range-slider]')) {
							$(dx[k]).slider('value', dx['attr']);
						} else {
							$(obPop).find(dx[k]).attr(dx['attr']);
						}
					}
				} else if (dx.hasOwnProperty('removeClass')) {
					$(obPop).find(dx[k]).removeClass(dx['removeClass']);
				} else if (dx.hasOwnProperty('class')) {
					$(obPop).find(dx[k]).addClass(dx['class']);
				} else if (dx.hasOwnProperty('val')) {
					$(obPop).find(dx[k]).val(dx['val']);
				} else if (dx.hasOwnProperty('text')) { // k === 'selector'
					if ($(obPop).find(dx[k]).length > 0) {
						$(obPop).find(dx[k]).text(dx['text']);
					}
				} else if (dx.hasOwnProperty('template')) { // k === 'selector'
					if ($(obPop).find(dx[k]).length > 0) {
	...
	close: function (obPop) {
		...
		// $(obPop).closest('.modalOverlay').removeClass('active').removeAttr('style');
		// $(obPop).removeClass('open');
		...
		if ($.trim(dataset.animateOut) !== '') {
			$(obPop).addClass(dataset.animateOut).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
				$(this).removeClass('open ' + dataset.class + ' ' + dataset.animateOut)
					.parent('.modalOverlay').removeClass('active').removeAttr('style');
			});
			popup.refresh();
		} else {
			$(obPop).removeClass('open ' + dataset.class).closest('.modalOverlay').removeClass('active').removeAttr('style');
		}
...
}; $(document).ready(function () { "use strict"; popup.init(); });
	...
	$.fn.fastInput = function (action) {
		var formControls = 'input,button,select,textarea',
			...
			findAccessibleElement = function (ob) {
				if (
					$($(accessibles)[$(accessibles).index(ob) + 1]).is(':hidden') ||
					$($(accessibles)[$(accessibles).index(ob) + 1]).is(':disabled') ||
					$($(accessibles)[$(accessibles).index(ob) + 1]).is('[readonly]') ||
					typeof ($($(accessibles)[$(accessibles).index(ob) + 1])) === undefined
				) {
					// console.log($($(accessibles)[$(accessibles).index(ob) + 1]));
					return findAccessibleElement($($(accessibles)[$(accessibles).index(ob) + 1]));
				} else {
					// console.log($($(accessibles)[$(accessibles).index(ob) + 1]));
					return $($(accessibles)[$(accessibles).index(ob) + 1]);
				}
			},
			searchFormCtrl = function (ob) {
				if (
					$($(formControls)[$(formControls).index(ob) + 1]).is(':hidden') ||
					$($(formControls)[$(formControls).index(ob) + 1]).is(':disabled') ||
					$($(formControls)[$(formControls).index(ob) + 1]).is('[readonly]') ||
					typeof ($($(formControls)[$(formControls).index(ob) + 1])) === undefined
				) {
					// console.log($($(formControls)[$(formControls).index(ob) + 1]));
					return searchFormCtrl($($(formControls)[$(formControls).index(ob) + 1]));
				} else {
					// console.log($($(formControls)[$(formControls).index(ob) + 1]));
					return $($(formControls)[$(formControls).index(ob) + 1]);
				}
			},
		...
		if (action === "init") {
			$(this).off('keydown.fastInput')
				.on('keydown.fastInput', '.form-control, input, select',
				function (e) {
					switch (e.which) {
						case kb.ENTER:
